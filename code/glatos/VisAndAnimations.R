...
make_transition_with_invert <- function(in_file, output = "out.tif",
...
#add to sysdata.rda
#rda_file <- file.path("Work/glatos/R/sysdata.rda")
...
abacus_plot(detections_filtered[detections_filtered$animal_id== "NSBS-Hooker",],
            location_col='station',
            main="NSBS-Hooker Detections By Station")
...
?make_transition
tran <- make_transition_with_invert(halifax, res = c(0.1, 0.1), invert=TRUE)

plot(tran$rast, xlim = c(-66, -62), ylim = c(42, 45))
plot(halifax, add = TRUE)

# not high enough resolution- bump up resolution
tran1 <- make_transition_with_invert(halifax, res = c(0.001, 0.001), invert=TRUE)
?min_lag
